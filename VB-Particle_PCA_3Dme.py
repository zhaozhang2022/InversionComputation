...
qwt_real=np.loadtxt('realqwt_imp_noisefixedsigma_dt360knt150.txt')
...
def singlephase_unsteady_imp(kvec): #homogeneous reservoir input k output transient p of bottomleft corner producer
    ...
    qwt=np.zeros((4,nt))   # record flow rate with time for 3D 四口井
    ...
    for t in range(nt):
        for ie in range(ncell):
            ...
            if celllist[icell].markwell > 0:
                ...
                RHS[ie] = RHS[ie] - qw
                qwt[celllist[icell].markwell - 1, t] = qw
            for j in range(6):
...
f = open('writeELBO_pca80_50samples-10updates.txt','w')
...
f = open('distributions_pca80_50samples-10updates.txt','w')
f.write("result mean: \n")
for i in range(ndim):
    f.write("%e " % (qmean[i]))
f.write("\n")
...
f = open('allY_pca80_50samples_10updates.txt','w')
for i in range(nsamples):
    for j in range(ndim):
        f.write("%e " % (mf[i, j]))
    f.write("\n")
f.close()
print("output to vtk")
f = open('result_pca80_50samples_10updates.vtk','w')
